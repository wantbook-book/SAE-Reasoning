...
def convert_hf_model_config(model_name: str, **kwargs):
    ...
    if "deepseek-ai" in official_model_name.lower():
        ...
        deepseek_model_name = official_model_name
        # modify the official model (LOCALLY here) to the source model
        if 'Qwen-1.5B' in official_model_name:
            official_model_name = "Qwen/Qwen2.5-1.5B"
        elif 'Qwen-7B' in official_model_name:
            official_model_name = "Qwen/Qwen2.5-7B"
        elif 'Llama-8B' in official_model_name:
            official_model_name = "meta-llama/Llama-3.1-8B"
        else:
            raise NotImplementedError("Unknown deepseek instance.")
    ...
    if official_model_name.startswith(
        ...
        cfg_dict = {
            ...
            "d_mlp": 14336,
            "n_layers": 32,
            "n_ctx": 16384,  # capped due to memory issues
            "eps": 1e-5,
            "d_vocab": 128256,
            "act_fn": "silu",
            "n_key_value_heads": 8,
        ...
        cfg_dict = {
            ...
            "d_mlp": hf_config.intermediate_size,
            "n_layers": hf_config.num_hidden_layers,
            "n_ctx": 16384,  # Capped bc the actual ctx length is 30k and the attn mask would be too big
            "eps": hf_config.rms_norm_eps,
            "d_vocab": hf_config.vocab_size,
            "act_fn": hf_config.hidden_act,
            "use_attn_scale": True,
    ...
    cfg_dict["tokenizer_name"] = official_model_name if deepseek_model_name is None else deepseek_model_name
