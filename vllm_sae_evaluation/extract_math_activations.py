...
import h5py
import numpy as np
from utils.sae_utils import add_hooks, get_intervention_hook, get_clamp_hook, get_multi_intervention_hook, activation_logger
...
def parse_args():
    ...
    # Activation logging arguments
    parser.add_argument(
        "--enable_activation_logging",
        action="store_true",
        help="Enable activation logging to save SAE activations before and after intervention to .pt files.",
    )
...
def setup(args):
    ...
    if args.use_vllm:
        ...
        # Log activation logging status
        if args.enable_activation_logging:
            print("Activation logging: ENABLED - SAE activations before and after intervention will be saved to .pt files")
        else:
            print("Activation logging: DISABLED")
...
def main(llm, tokenizer, sae_hooks, data_name, args):
    ...
    if len(processed_samples) < len(all_samples) and args.save_outputs:
        ...
        # Save activation data to h5 file if logging was enabled
        if args.enable_activation_logging and activation_logger.has_data():
            h5_path = out_file.replace('.jsonl', '_activations.h5')
            ...
            with h5py.File(h5_path, 'w') as f:
                ...
                # Save metadata
                f.attrs['timestamp'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ...
            # Clear activation data after saving
            activation_logger.clear_data()
