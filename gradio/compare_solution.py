...
class JSONLComparator:
    ...
    def apply_filter(self):
        ...
        if self.filter_mode == "all":
            ...
            for idx in self.all_indices:
                ...
                if (item1 and item2 and 
                    item1.get('code_evaluations') and item2.get('code_evaluations') and
                    len(item1['code_evaluations']) > 0 and len(item2['code_evaluations']) > 0):
                    
                    is_correct1 = item1['code_evaluations'][0].get('is_correct', False)
                    is_correct2 = item2['code_evaluations'][0].get('is_correct', False)
    ...
    def get_current_data(self):
        ...
        # 获取代码回答
        code1 = item1.get('code', [''])[0] if item1.get('code') else '无代码回答'
        code2 = item2.get('code', [''])[0] if item2.get('code') else '无代码回答'
        
        # 获取评估结果
        eval1 = item1.get('code_evaluations', [{}])[0] if item1.get('code_evaluations') else {}
        eval2 = item2.get('code_evaluations', [{}])[0] if item2.get('code_evaluations') else {}
        
        is_correct1 = eval1.get('is_correct', False)
        is_correct2 = eval2.get('is_correct', False)
        
        extracted_answer1 = str(eval1.get('extracted_answer', '无提取答案'))
        extracted_answer2 = str(eval2.get('extracted_answer', '无提取答案'))
...
with gr.Blocks(title="JSONL文件对比工具") as demo:
    ...
    # 第五行：显示两个文件的代码回答
    with gr.Row():
        code1_display = gr.Textbox(label="文件1代码回答", interactive=False, lines=10, max_lines=20, show_copy_button=True)
        code2_display = gr.Textbox(label="文件2代码回答", interactive=False, lines=10, max_lines=20, show_copy_button=True)
